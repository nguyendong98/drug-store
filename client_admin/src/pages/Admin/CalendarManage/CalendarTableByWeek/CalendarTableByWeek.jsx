...
export const CalendarTableByWeek = ({workShifts, calendars, typeTable, week}) => {
    let weekToDate = new Date(week);
    console.log(week);
    weekToDate = new Date(weekToDate.setDate(weekToDate.getDate() + 6));
    const dataWeek = [];
    for(let i = 0; i < 7; i++) {
        dataWeek.push(moment(new Date(week)).add(i, 'days'))
    }
    ...
    return typeTable === 2 && (
        <>
            <Typography variant="h6" className="text-center font-weight-bold mb-8">
                Lịch làm việc từ {moment(new Date(week)).format('DD/MM/YYYY')} ~ {moment(new Date(weekToDate)).format('DD/MM/YYYY')}
            </Typography>
